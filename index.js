...
async function run() {
  try {
    ...
    const containerName = core.getInput('override-container-name', { required: false });
    const command = core.getInput('override-command', { required: false });
    ...
    core.debug(`Running task with ${JSON.stringify(runTaskArgs)}`)

    const runTaskResponse = await ecs.runTask(runTaskArgs).promise();
